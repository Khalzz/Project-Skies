...
use cgmath::{Deg, ElementWise, Euler, InnerSpace, Point3, Quaternion, Rad, Rotation, Rotation3, Vector2, Vector3, Zero};
use glyphon::Color;
...
impl GameLogic {
    ...
    fn ui_control(&mut self, app: &mut App, delta_time: f32) {
        ...
        // let rotation = Self::map_to_range(plane_rotation.z.0.into(), -PI, PI, 0.0, 360.0).round();
        // let (axis, angle) = Self::quaternion_to_axis_angle(plane.rotation);
        app.components.get_mut("horizon").unwrap().rectangle.rotation = plane.rotation;
        app.components.get_mut("horizon").unwrap().rectangle.color[3] = 0.0;
    ...
    fn quaternion_to_euler_angles(quat: Quaternion<f32>) -> (Deg<f32>, Deg<f32>, Deg<f32>) {
        // Convert the quaternion to Euler angles in radians
        let euler: Euler<Rad<f32>> = quat.into();
        
        // Convert radians to degrees
        let x = Deg::from(euler.x);
        let y = Deg::from(euler.y);
        let z = Deg::from(euler.z);
